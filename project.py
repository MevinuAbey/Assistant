...
def get_command():
    try:
        ...
        output("No command detected (timeout).")
        return None
    except sr.UnknownValueError:
        output("Could not understand the audio.")
        return None
    except sr.RequestError as e:
        output(f"Speech recognition error: {e}")
        return None
...
def main():
    ...
    while listening:
        ...
        if command.startswith("play"):
            ...
            output("Sorry, I can't understand that. Please try again.")
...
root = tk.Tk()
root.title("Voice Assistant 1.0 - Mevinu Abeysinghe")
